  ...
  def relationship_query
    <<-SQL
        WITH RECURSIVE deep_members AS (
          ...
          select member.id AS file_set_id, event.metadata->'resource_id'->0->>'id' AS id, event.metadata->'child_id'->0->>'id' AS child_id, event.metadata->'status'->>0 AS status, event.updated_at
        ...
        select COUNT(DISTINCT deep_events.file_set_id) FROM (
          select file_set_id, id, child_id, MAX(event.updated_at) AS updated_at FROM deep_events event
          GROUP BY file_set_id, id, child_id
        ) AS latest_events
