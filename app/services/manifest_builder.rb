  ...
  class LeafNode
    ...
    def download_url
      ...
      if helper.token_authorizable?(parent_node.resource)
        helper.download_url(resource.try(:proxied_object_id) || resource.id, derivative.id, type: "stream", auth_token: parent_node.resource.auth_token)
      else
        helper.download_url(resource.try(:proxied_object_id) || resource.id, derivative.id, type: "stream")
      end
    end
