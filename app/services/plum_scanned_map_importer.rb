...
class PlumScannedMapImporter
  ...
  def import!
    ...
    add_thumbnail_id(output, members, change_set_persister)
    update_structure(output, members, change_set_persister)
    apply_extra_attributes(output, change_set_persister)
  end

  def add_thumbnail_id(resource, members, change_set_persister)
    change_set = ScannedMapChangeSet.new(resource).tap(&:prepopulate!)
    file_sets = members.select { |x| x.is_a?(FileSet) }
    scanned_map_members = members.select { |x| x.is_a?(ScannedMap) }
    thumbnail_id = nil
    if !scanned_map_members.empty?
      scanned_map = scanned_map_members.first.decorate
      thumbnail_id = scanned_map.geo_members&.first.try(:id)
    elsif !file_sets.empty?
      thumbnail_id = file_sets.first.id
    end
    change_set.validate(thumbnail_id: thumbnail_id)
    change_set.sync
    change_set_persister.save(change_set: change_set)
  end

  def apply_extra_attributes(resource, change_set_persister)
    change_set = ScannedMapChangeSet.new(resource).tap(&:prepopulate!)
    change_set.validate(extra_attributes)
    change_set.sync
    change_set_persister.save(change_set: change_set)
  end
  ...
  def change_set
    @change_set ||= ScannedMapChangeSet.new(resource).tap do |change_set|
  ...
  def resource
    @resource ||= ScannedMap.new(document.attributes)
  end
  ...
  class PlumDocument
    ...
    def local_identifier
      replaces = solr_doc.fetch("replaces_ssim", []).first
      [id, replaces]
    end

    def title
      solr_doc.fetch("title_tesim", [])
    end
    ...
    def coverage
      solr_doc.fetch("coverage_ssim", []).first
    end

    def creator
      solr_doc.fetch("creator_tesim", [])
    end

    def publisher
      solr_doc.fetch("publisher_tesim", [])
    end

    def spatial
      solr_doc.fetch("spatial_ssim", [])
    end

    def subject
      solr_doc.fetch("subject_tesim", [])
    end

    def description
      solr_doc.fetch("description_tesim", [])
    end

    def temporal
      solr_doc.fetch("temporal_ssim", [])
    end
    ...
    class Children
      ...
      def to_a
        @to_a ||= child_documents.map do |id|
          child_importer = PlumScannedMapImporter.new(id: id, change_set_persister: change_set_persister)
      ...
      def to_a
        file_set_documents.map do |file_set|
          IngestableFile.new(
            ...
            mime_type: file_set.geo_mime_type,
  ...
  class FileSetDocument
    ...
    def geo_mime_type
      Array.wrap(solr_doc["geo_mime_type_ssim"]).first || "application/xml; schema=fgdc"
    end
  end
