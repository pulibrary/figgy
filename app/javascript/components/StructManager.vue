...
export default {
  ...
  methods: {
    addGalleryItems: function () {
      const galleryItems = JSON.parse(JSON.stringify(this.gallery.items)).concat(this.changeKeyToCaption(this.end_nodes))
    ...
    addNewNode: function (array, newParent) {
      for (let item of array) {
        if (item.id === newParent.id) {
          ...
          this.addNewNode(item.folders, newParent)
        }
      }
      return array
    },
    changeKeyToCaption: function (array) {
      ...
      for (let i = 0; i < array.length; i++) {
        ...
        if (Object.prototype.hasOwnProperty.call(array[i], 'label')) {
    ...
    deleteFolder: function (folderId) {
      const folderList = JSON.parse(JSON.stringify(this.tree.structure.folders))
      const folderToBeRemoved = this.findFolderById(folderList, folderId)
      ...
      if (folderToBeRemoved.folders.length) {
        ...
        const text = 'This folder contains subfolders, which will be removed by this action. Do you still want to proceed?'
    ...
    filterGallery: function (newVal) {
      if (this.structure.nodes) {
        ...
        const ga = JSON.parse(JSON.stringify(this.galleryItems))
    ...
    groupSelectedIntoFolder: function () {
      this.cutSelected()
      this.$nextTick(() => {
        const folderId = this.createFolder(this.tree.structure.id)
    ...
    pasteGalleryItem: function () {
      ...
      const resources = JSON.parse(JSON.stringify(this.gallery.cut))
    ...
    zoomFile: function (fileId) {
      const folderList = JSON.parse(JSON.stringify(this.tree.structure.folders))
      const selected = this.findFolderById(folderList, fileId)
    ...
    zoomOnItem: function () {
      ...
      if (this.tree.selected) {
        const folderList = JSON.parse(JSON.stringify(this.tree.structure.folders))
        const nodeToBeZoomed = this.findFolderById(folderList, this.tree.selected)
