...
test("doesn't break when there's no folder selected", () => {
  mount(DirectoryContents, { propsData: { folder: null, mode: 'directoryIngest' } })
})
...
test('in fileIngest mode it renders an Ingest selected files button', async () => {
  const wrapper = mount(DirectoryContents, { propsData: { folder: folder(), mode: 'fileIngest' } })
...
test("in fileIngest selectAll is disabled if there's no files", async () => {
  const wrapper = mount(DirectoryContents, { propsData: { folder: disabledFolder(), mode: 'fileIngest' } })
