...
RSpec.describe HumanReadableTypeIndexer do
  describe ".to_solr" do
    ...
    context 'when a scanned resource has multiple volumes' do
      let(:child_volume) { FactoryGirl.create_for_repository(:scanned_resource) }
      let(:scanned_resource) { FactoryGirl.create_for_repository(:scanned_resource, member_ids: [child_volume.id]) }
