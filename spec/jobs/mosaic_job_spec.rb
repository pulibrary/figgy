...
describe MosaicJob do
  ...
  let(:mosaic_service) { instance_double(MosaicService) }

  before do
    allow(mosaic_service).to receive(:path)
    allow(MosaicService).to receive(:new).and_return(mosaic_service)
  end

  context "with a raster set resource" do
    it "runs the MosaicService" do
  ...
  context "with a non-set raster resource" do
    it "returns without calling the MosaicService" do
