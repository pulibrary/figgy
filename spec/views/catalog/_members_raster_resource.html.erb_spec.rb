...
RSpec.describe "catalog/_members_raster_resource" do
  context 'when the RasterResource has members' do
    let(:child) { FactoryBot.create_for_repository(:raster_resource, title: 'vol1', rights_statement: 'x') }
    let(:parent) { FactoryBot.create_for_repository(:raster_resource, title: 'Mui', rights_statement: 'y', member_ids: [child.id]) }
