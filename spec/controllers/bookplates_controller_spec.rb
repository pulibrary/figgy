...
RSpec.describe BookplatesController do
  ...
  context "when they have permission" do
    ...
    it "has a form for creating scanned resources" do
      ...
      parent = FactoryBot.create_for_repository(:bookplate)
      ...
      expect(response.body).to have_selector "#bookplate_append_id[value='#{parent.id}']", visible: false
      expect(response.body).to have_select "Collections", name: "bookplate[member_of_collection_ids][]", options: [collection.title.first]
      expect(response.body).to have_select "Rights Statement", name: "bookplate[rights_statement]", options: [""] + ControlledVocabulary.for(:rights_statement).all.map(&:label)
      expect(response.body).to have_select "PDF Type", name: "bookplate[pdf_type]", options: ["Color PDF", "Grayscale PDF", "Bitonal PDF", "No PDF"]
  ...
  describe "create" do
    ...
    it "can create a scanned resource" do
      post :create, params: { bookplate: valid_params }
    ...
    it "renders the form if it doesn't create a scanned resource" do
      post :create, params: { bookplate: invalid_params }
  ...
  describe "destroy" do
    let(:user) { FactoryBot.create(:admin) }
    context "access control" do
      let(:factory) { :bookplate }
      it_behaves_like "an access controlled destroy request"
    end
    it "can delete a book" do
      bookplate = FactoryBot.create_for_repository(:bookplate)
      delete :destroy, params: { id: bookplate.id.to_s }

      expect(response).to redirect_to root_path
      expect { query_service.find_by(id: bookplate.id) }.to raise_error ::Valkyrie::Persistence::ObjectNotFoundError
    end
  end
  ...
  describe "edit" do
    ...
    context "when it does exist" do
      ...
      it "renders a form" do
        bookplate = FactoryBot.create_for_repository(:bookplate)
        get :edit, params: { id: bookplate.id.to_s }

        expect(response.body).to have_field "Title", with: bookplate.title.first
        expect(response.body).to have_button "Save"
      end
    end
  end
  ...
  describe "html update" do
    ...
    context "html" do
      context "access control" do
        let(:factory) { :bookplate }
        let(:extra_params) { { bookplate: { title: ["Two"] } } }
      ...
      context "when it does exist" do
        it "saves it and redirects" do
          bookplate = FactoryBot.create_for_repository(:bookplate)
          patch :update, params: { id: bookplate.id.to_s, bookplate: { title: ["Two"] } }

          expect(response).to be_redirect
          expect(response.location).to eq "http://test.host/catalog/#{bookplate.id}"
        ...
        it "renders the form if it fails validations" do
          bookplate = FactoryBot.create_for_repository(:bookplate)
          patch :update, params: { id: bookplate.id.to_s, bookplate: { title: [""] } }
    ...
    context "json" do
      context "when not an admin" do
        ...
        it "returns 403" do
          resource = FactoryBot.create_for_repository(:bookplate)
          params = { id: resource.id.to_s, bookplate: { member_ids: ["not_an_id"] }, format: :json }
      ...
      context "when the scanned resource does exist" do
        context "invalid data submitted" do
          it "returns 400" do
            bookplate = FactoryBot.create_for_repository(:bookplate)
            params = { id: bookplate.id.to_s, bookplate: { title: [""] }, format: :json }
            patch :update, params: params
        ...
        context "valid data submitted" do
          it "updates and returns 200" do
            ...
            bookplate = FactoryBot.create_for_repository(:bookplate, member_ids: [file_set1.id, file_set2.id])

            params = { id: bookplate.id.to_s, bookplate: { member_ids: [file_set2.id, file_set1.id] }, format: :json }
            patch :update, params: params
            expect(response.status).to eq(200)
            reloaded = find_resource(bookplate.id)
  ...
  describe "GET /concern/bookplates/:id/manifest" do
    let(:file) { fixture_file_upload('files/example.tif', 'image/tiff') }
    it "returns a IIIF manifest for a resource with a file" do
      bookplate = FactoryBot.create_for_repository(:bookplate, files: [file])

      get :manifest, params: { id: bookplate.id.to_s, format: :json }
    ...
    context "when given a local identifier" do
      it "returns it still" do
        bookplate = FactoryBot.create_for_repository(:bookplate, local_identifier: "pk643fd004", files: [file])

        get :manifest, params: { id: bookplate.local_identifier.first, format: :json }

        expect(response).to redirect_to manifest_bookplate_path(id: bookplate.id.to_s)
      end
    end
  end

  describe "GET /concern/bookplates/:id/pdf" do
    let(:file) { fixture_file_upload('files/example.tif', 'image/tiff') }
    let(:bookplate) { FactoryBot.create_for_repository(:bookplate, files: [file]) }
    let(:file_set) { bookplate.member_ids.first }
    before do
    ...
    it "generates a PDF, attaches it to the Bookplate, and redirects to download for it", fail: true do
      get :pdf, params: { id: bookplate.id.to_s }
      reloaded = adapter.query_service.find_by(id: bookplate.id)

      expect(reloaded.file_metadata).not_to be_blank
      expect(reloaded.pdf_file).not_to be_blank
      expect(response).to redirect_to Valhalla::Engine.routes.url_helpers.download_path(resource_id: bookplate.id.to_s, id: reloaded.pdf_file.id.to_s)
    end
  end
