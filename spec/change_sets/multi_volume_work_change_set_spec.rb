...
RSpec.describe MultiVolumeWorkChangeSet do
  subject(:change_set) { described_class.new(form_resource) }
  let(:multi_volume_work) { MultiVolumeWork.new(title: 'Test', rights_statement: 'Stuff', visibility: Hydra::AccessControls::AccessRight::VISIBILITY_TEXT_VALUE_PRIVATE, state: 'pending') }
  let(:form_resource) { multi_volume_work }
  before do
  ...
  describe "validations" do
    ...
    context "when neither title or metadata identifier is set" do
      let(:form_resource) { multi_volume_work.new(title: "", source_metadata_identifier: "") }
      it "is invalid" do
        expect(change_set).not_to be_valid
      end
    end
    ...
    context "when only metadata_identifier is set" do
      let(:form_resource) { multi_volume_work.new(title: "", source_metadata_identifier: "123456") }
      it "is valid" do
        expect(change_set).to be_valid
      end
    end
    context "when rights_statement isn't set" do
      let(:form_resource) { multi_volume_work.new(rights_statement: [""]) }
      it "is invalid" do
        expect(change_set).not_to be_valid
      end
    end
    context "when visibility isn't set" do
      let(:form_resource) { multi_volume_work.new(visibility: [""]) }
      it "is invalid" do
        expect(change_set).not_to be_valid
      end
    end
  ...
  describe "#viewing_hint" do
    it "is singular" do
      multi_volume_work.viewing_hint = ["Test"]
  ...
  describe "#viewing_direction" do
    it "is singular" do
      multi_volume_work.viewing_direction = ["Test"]
      change_set.prepopulate!
  ...
  describe "#pdf_type" do
    let(:form_resource) { MultiVolumeWork.new }
  ...
  describe "#rights_statement" do
    let(:form_resource) { MultiVolumeWork.new(rights_statement: RDF::URI("http://rightsstatements.org/vocab/NKC/1.0/")) }
