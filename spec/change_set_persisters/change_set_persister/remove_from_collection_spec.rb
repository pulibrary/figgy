...
RSpec.describe ChangeSetPersister::RemoveFromCollection do
  ...
  it "stops highlighting the resource in the collections it was removed from" do
    collection1 = FactoryBot.create_for_repository(:collection)
    ...
    resource = FactoryBot.create_for_repository(
      ...
      featurable: [collection1.id, collection2.id]
    )
    change_set = ChangeSet.for(resource)
    ...
    expect(output.featurable.map(&:to_s)).to eq [collection2.id.to_s]
  end
end
