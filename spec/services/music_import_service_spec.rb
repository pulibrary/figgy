  ...
  describe "#ingest_recording" do
    ...
    context "when a recording is a faux-playlist" do
      it "ingests the actual recording and just creates selections" do
        ...
        dependent_collector = instance_double(MusicImportService::RecordingCollector)
        allow(importer.recording_collector).to receive(:with_recordings_query).and_return(dependent_collector)
        allow(importer.recording_collector).to receive(:audio_files).with(recording2).and_return(audio_files)
        allow(importer.recording_collector).to receive(:dependent_recordings_query).with([15])
        allow(dependent_collector).to receive(:recordings).and_return([recording2])
