  ...
  describe "#base_url", run_real_derivatives: true do
    context "with generated derivatives" do
      it "generates a base URL for a pyramidal if it's present" do
        ...
        path = path.gsub(Figgy.config["pyramidal_derivative_path"], "").gsub(/^\//, "")
        expect(cantaloupe_helper.base_url(file_set)).to eq "http://localhost:8182/iiif/2/pyramidals%2F#{path.gsub('/', '%2F')}"
      end
