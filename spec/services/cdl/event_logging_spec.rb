...
RSpec.describe CDL::EventLogging do
      ...
      before do
        stub_request(:get, "https://bibdata.princeton.edu/patron/#{netid}")
          .to_return(status: 200,
                     body: file_fixture("files/catalog/#{netid}.json").read, headers: { "Content-Type" => "application/json" })
      end
  ...
  describe ".google_hold_expired_event" do
    let(:netid) { "abc123" }
    let(:bibid) { "12345" }
    before do
      stub_request(:get, "https://bibdata.princeton.edu/patron/#{netid}")
    ...
    it "sends a hold-expired event to google analytics" do
      described_class.google_hold_expired_event(source_metadata_identifier: bibid, netid: netid)
      expect(a_request(:post, "https://www.google-analytics.com/collect")
      .with(body: "cid=1&ea=hold-expired&ec=CDL-staff&el=12345&t=event&tid=UA-15870237-29&ua=Figgy&v=1")).to have_been_made
    end
  end
end
